...
public class GameControllerImpl implements GameController {
    ...
    public GameStatusDto joinGame(final String code, final UUID playerId) {
        ...
        boolean allReadyPlayersJoined = room.getPlayers().values().stream().allMatch(p -> p.getStatus() == PlayerStatus.IN_GAME);
        ...
        return GameStatusDto.builder()
            ...
            .currentQuestion(QuestionDto.from(room.getGame().getCurrentQuestion(), room.getGame().getCurrentQuestionOrdinal()))
    ...
    @Override
    public QuestionResultDto answerQuestion(final String code, final UUID playerId, final PlayerQuestionResponseDto response) {
        ...
        if (response == null || response.questionId() == null || response.responseId() == null || response.milliseconds() == 0) {
        ...
        List<Player> orderedPlayers = playersAnswered.stream().sorted((p1, p2) -> (int) (p1.getResponseTime() - p2.getResponseTime())).toList();
    ...
    public GameStatusDto nextQuestion(final String code, final UUID playerId) {
        ...
        boolean allReadyPlayersNext = room.getPlayers().values().stream().allMatch(p -> p.isReadyForNextQuestion());
        ...
        boolean thereAreMoreQuestions = room.getGame().getCurrentQuestionOrdinal() < room.getNumQuestions();
